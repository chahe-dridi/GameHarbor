...
class ContractCreateFlow {
  ...
  async execute(client, requestTimeout) {
    ...
    if (this._bytecode.length > 2048) {
      const fileAppendTransaction = new _FileAppendTransaction.default().setFileId(fileId).setContents(this._bytecode.subarray(2048)).freezeWith(client);
    ...
    if (key != null) {
      const fileDeleteTransaction = new _FileDeleteTransaction.default().setFileId(fileId).freezeWith(client);
  ...
  async executeWithSigner(signer) {
    ...
    if (key instanceof _PublicKey.default) {
      ...
      formattedPublicKey = _PublicKey.default.fromBytes(keyArray);
    }
    const fileCreateTransaction = await new _FileCreateTransaction.default().setKeys(formattedPublicKey != null ? [formattedPublicKey] : []).setContents(this._bytecode.subarray(0, Math.min(this._bytecode.length, 2048))).freezeWithSigner(signer);
    ...
    const fileId = /** @type {FileId} */receipt.fileId;
    if (this._bytecode.length > 2048) {
      let fileAppendTransaction = new _FileAppendTransaction.default().setFileId(fileId).setContents(this._bytecode.subarray(2048));
